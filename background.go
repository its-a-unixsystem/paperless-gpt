...
type BackgroundProcessor interface {
	...
	processAutoOneshotDocuments(ctx context.Context) (int, error)
	isOcrEnabled() bool
	isOneshotEnabled() bool
}
...
func StartBackgroundTasks(ctx context.Context, app BackgroundProcessor) {
	go func() {
		...
		for {
			...
			processedCount, err := runBackgroundCycle(ctx, app)
...
func runBackgroundCycle(ctx context.Context, app BackgroundProcessor) (int, error) {
	count := 0
	var errs []error
	...
	if app.isOcrEnabled() {
		...
		count += ocrCount
		if err != nil {
			errs = append(errs, fmt.Errorf("error in processAutoOcrTagDocuments: %w", err))
		}
	}
	...
	count += autoCount
	if err != nil {
		errs = append(errs, fmt.Errorf("error in processAutoTagDocuments: %w", err))
	}

	// Run oneshot processing independently.
	if app.isOneshotEnabled() {
		oneshotCount, err := app.processAutoOneshotDocuments(ctx)
		count += oneshotCount
		if err != nil {
			errs = append(errs, fmt.Errorf("error in processAutoOneshotDocuments: %w", err))
		}
	}
...
func (app *App) processAutoTagDocuments(ctx context.Context) (int, error) {
	...
	for _, docSummary := range documents {
		...
		// Skip documents that have the autoOneshotTag (handled by oneshot pipeline)
		if autoOneshotTag != "" && slices.Contains(document.Tags, autoOneshotTag) {
			log.Debugf("Skipping document %d as it has the oneshot tag %s", document.ID, autoOneshotTag)
			continue
		}
...
// isOneshotEnabled returns true if oneshot mode is enabled
func (app *App) isOneshotEnabled() bool {
	return oneshotEnable
}

// processAutoOneshotDocuments handles the background oneshot processing of documents
func (app *App) processAutoOneshotDocuments(ctx context.Context) (int, error) {
	documents, err := app.Client.GetDocumentsByTags(ctx, []string{autoOneshotTag}, 25)
	if err != nil {
		return 0, fmt.Errorf("error fetching documents with autoOneshotTag: %w", err)
	}

	if len(documents) == 0 {
		log.Debugf("No documents with tag %s found", autoOneshotTag)
		return 0, nil
	}

	// Refresh the custom fields cache before processing
	refreshCustomFieldsCache(app.Client)

	log.Debugf("Found %d documents with tag %s", len(documents), autoOneshotTag)
	...
	for _, docSummary := range documents {
		...
		docLogger := documentLogger(document.ID)
		docLogger.Info("Processing document for oneshot")
		...
		suggestions, err := app.generateDocumentSuggestions(ctx, suggestionRequest, docLogger)
		if err != nil {
			// On permanent error, tag with error tag and remove oneshot tag to stop retrying
			if isPermanentOneshotError(err) {
				docLogger.Warnf("Permanent oneshot error for document %d, tagging with %s: %v", document.ID, oneshotErrorTag, err)
				tagErr := app.Client.UpdateDocuments(ctx, []DocumentSuggestion{
					{
						ID:               document.ID,
						OriginalDocument: document,
						RemoveTags:       []string{autoOneshotTag, autoTag},
						AddTags:          []string{oneshotErrorTag},
					},
				}, app.Database, false)
				if tagErr != nil {
					docLogger.Errorf("Failed to apply error tag to document %d: %v", document.ID, tagErr)
					errs = append(errs, fmt.Errorf("failed to apply oneshot error tag for document %d: %w", document.ID, tagErr))
					continue
				}
				processedCount++
				continue
		...
		docLogger.Info("Successfully processed document via oneshot")
		processedCount++
	}
