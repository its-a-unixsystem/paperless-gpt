...
func (p *GoogleAIProvider) generateText(ctx context.Context, prompt string, opts ...llms.CallOption) (string, error) {
	resp, err := p.client.Models.GenerateContent(ctx, p.model, genai.Text(prompt), p.buildGenerateContentConfig(opts...))
	if err != nil {
...
func (p *GoogleAIProvider) buildGenerateContentConfig(opts ...llms.CallOption) *genai.GenerateContentConfig {
	...
	if callOpts.CandidateCount >= 0 {
		config.CandidateCount = int32(callOpts.CandidateCount)
		hasConfig = true
	}
	if callOpts.MaxTokens >= 0 {
		config.MaxOutputTokens = int32(callOpts.MaxTokens)
		hasConfig = true
	}
	if !math.IsNaN(callOpts.Temperature) {
		temp := float32(callOpts.Temperature)
		config.Temperature = &temp
		hasConfig = true
	}
	if callOpts.TopK >= 0 {
		topK := float32(callOpts.TopK)
		config.TopK = &topK
		hasConfig = true
	}
	if !math.IsNaN(callOpts.TopP) {
		topP := float32(callOpts.TopP)
		config.TopP = &topP
		hasConfig = true
	}
	if len(callOpts.StopWords) > 0 {
		config.StopSequences = callOpts.StopWords
		hasConfig = true
	}
...
func (p *GoogleAIProvider) GenerateContent(ctx context.Context, messages []llms.MessageContent, opts ...llms.CallOption) (*llms.ContentResponse, error) {
	...
	resp, err := p.client.Models.GenerateContent(ctx, p.model, []*genai.Content{{Parts: parts}}, p.buildGenerateContentConfig(opts...))
	if err != nil {
...
func (p *GoogleAIProvider) Call(ctx context.Context, prompt string, opts ...llms.CallOption) (string, error) {
	return p.generateText(ctx, prompt, opts...)
}
