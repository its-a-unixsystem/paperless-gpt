...
func TestGoogleAIProvider_BuildGenerateContentConfig_Empty(t *testing.T) {
	provider := &GoogleAIProvider{}
	config := provider.buildGenerateContentConfig()
	assert.Nil(t, config)
}

func TestGoogleAIProvider_BuildGenerateContentConfig_ThinkingOnly(t *testing.T) {
	budget := int32(2048)
	provider := &GoogleAIProvider{
	...
	assert.NotNil(t, config.ThinkingConfig.ThinkingBudget)
	assert.Equal(t, budget, *config.ThinkingConfig.ThinkingBudget)
}

func TestGoogleAIProvider_BuildGenerateContentConfig_UsesCallOptions(t *testing.T) {
	provider := &GoogleAIProvider{}

	config := provider.buildGenerateContentConfig(
		llms.WithCandidateCount(2),
	...
	assert.NotNil(t, config)
	assert.Equal(t, int32(2), config.CandidateCount)
	...
	assert.Equal(t, float32(32), *config.TopK)
